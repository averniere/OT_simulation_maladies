...
from config import eta_list
from config import quantile_list
...
#Matrice de distance (via Jaccard), entre profil OMIM et profil complexe simulé
def Jaccard_distance_matrix(matrix1, matrix2):                              
    distances = pairwise_distances(matrix1, matrix2, metric='jaccard')
    return distances

#Optimal transport
def OT_Matrix(a, b, M, epsilon):
    """ 
    a = distribution de poids source
    b = distribution de poids target
    M = matrice de distance (via Jaccard_distance_matrix)
    epsilon = coeff de régularisation entropique 
    """
    if a is None:
        a = np.repeat(1./M.shape[0], M.shape[0])        #distribution uniforme des poids, toutes les maladies monogéniques ont les mêmes poids
    if b is None:
        b = np.repeat(1./M.shape[1], M.shape[1])        #distribution uniforme des poids, toutes les maladies complexes ont les mêmes poids

    Transport = sinkhorn(a, b, M, epsilon, method='sinkhorn', numItermax=1000, stopThr=1e-9,verbose=False, log=False, warn=True, warmstart=None)
...
def process_simulation(source_data, n_complex_list, n_match_list, noise_levels, quantiles, epsilon, overlap_test, group_size, eta_list):
        ...
        for n_complex in n_complex_list:
            for n_match in tqdm(n_match_list, desc="Testing different n_match values"):
                ...
                for noise_level in noise_levels:
                    ...
                    # Calcul des distances de Jaccard
                    jaccard_distances = Jaccard_distance_matrix(source_data_filtre.values, noisy_matrix.values)
                    jaccard_distances_df = pd.DataFrame(1 - jaccard_distances, index=source_data_filtre.index, columns=target_data.index)

                    #Raw distance
                    OT_type = "Raw"
                    jaccard_distances_results = filter_by_quantile(jaccard_distances_df, quantiles, n_match, OT_type, noise_level, overlap_rate, n_complex)
                    ...
                    #Optimal transport
                    transport_matrix = OT_Matrix(a, b, jaccard_distances, epsilon)
                    ...
                    for eta in eta_list:
                        for S_name, S_value in complementary_matrices.items():
                            ...
                            gamma_opt = Ot_Laplacienne(a, b, xs=Xs_real, xt=Xt_simu, M=jaccard_distances, S=S_value, epsilon=epsilon, eta=eta)
